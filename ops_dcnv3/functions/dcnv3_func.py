...
class DeformPoolFunction(Function):
    @staticmethod
    @custom_fwd
    def forward(
            ctx, input, offset, mask,
        ...
        ctx.offset_scale = offset_scale
        ctx.im2col_step = im2col_step
        output = DCNv3.deformpool_forward(
            input, offset, mask, kernel_h,
            kernel_w, stride_h, stride_w, pad_h,
            pad_w, dilation_h, dilation_w, group,
            group_channels, offset_scale, ctx.im2col_step)
...
def dcnv3_core_pytorch(
    ...
    #output = sampling_input_.sum(-1).view(N_, group*group_channels, H_out*W_out)
    output = sampling_input_.mean(dim=-1)
    output = output.view(N_, group*group_channels, H_out*W_out)
