...
class FusedFeatureLoader(Point3DLoader):
    ...
    def __init__(self,
        ...
        ## change val set
        if scan_refer :
            scan_refer_data_list = []
            refer_data_paths = 'data/openscene/scanrefer/'
            with open(os.path.join(refer_data_paths, 'ScanRefer_filtered_val.txt'), 'r') as file :
                refer_data_list = file.read()
            refer_data_list = refer_data_list.split('\n')
            origin_data_list = [origin_data_path.split('/')[4] for origin_data_path in self.data_paths]
            for ii, origin_data in enumerate(origin_data_list) :
                if origin_data[:12] in refer_data_list :
                    scan_refer_data_list.append(self.data_paths[ii])
            self.data_paths = scan_refer_data_list
