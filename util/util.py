...
def extract_target_landmark_feature(clip_pretrained, labelset, landmark, target, args) :
    # clip_pretrained, _ = clip.load(model_name, device='cuda', jit=False)
    labels = []

    for label in labelset :
        if label in target :
            labels.append(target + ' in a scene')
        elif label in landmark :
            labels.append(landmark + ' in a scene')
        else :
            labels.append('a ' + label + ' in a scene')
...
def extract_text_feature(labelset, args):
    ...
    if hasattr(args, 'prompt_eng') and args.prompt_eng:
        ...
        # labelset = [ "a wooden " + label + " in a scene" for label in labelset]
        # labelset = [ "a  " + label + " at left side of a door in a scene" for label in labelset]
